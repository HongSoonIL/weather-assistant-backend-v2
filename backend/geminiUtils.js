...
async function callGeminiForFinalResponse(userInput, toolSelectionResponse, toolOutputs, userProfile) {
  ...
  if (userProfile) {
    ...
    const hobbiesText = userProfile.hobbies?.length ? userProfile.hobbies.join(', ') : '정보 없음';
    const factorsText = userProfile.sensitiveFactors?.length ? userProfile.sensitiveFactors.join(', ') : '정보 없음';
  ...
  // 1차 호출에서 모델이 생성한 'functionCall' 부분을 가져옵니다.
  const modelResponse = toolSelectionResponse.candidates?.[0]?.content;
  if (!modelResponse) {
    throw new Error("1차 Gemini 응답에서 content 부분을 찾을 수 없습니다.");
  }

  // [핵심 수정] 도구가 사용되었을 때와 아닐 때를 구분하여 contents를 구성합니다.
  let contents = [];
  ...
  if (toolOutputs && toolOutputs.length > 0) {
    ...
    contents = [
      ...conversationStore.getHistory(),
      { role: 'user', parts: [{ text: userInput }] },
      modelResponse, // 모델의 함수 호출(functionCall) 기록
      {
        role: 'function',
  ...
  const systemInstruction  = {
    ...
    parts: [{ text: `
      ...
      - "공기질" 또는 "미세먼지": 'air' 데이터를 사용하여 초미세먼지(pm2.5) 수치를 기준으로 '좋음', '보통', '나쁨', '매우 나쁨' 단계로 설명해주고, 외출 시 마스크 착용 여부를 조언해줘.
  ...
  const { data } = await axios.post(
    ...
    {
      contents,
      systemInstruction, // ✨ 시스템 명령어를 요청에 포함시킵니다.
    }
  );
